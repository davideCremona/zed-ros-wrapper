...
{
  SimpleImageSubNodelet::~SimpleImageSubNodelet() {}

  void SimpleImageSubNodelet::onInit() {
    // Node handlers
    mNh = getNodeHandle();
    ...
    image_sub_ = it_->subscribe("/zed2/zed_node/left/image_rect_color", 1, &SimpleImageSubNodelet::imgCallback, this);
    NODELET_INFO_STREAM(" * Subscribed to topic: " << image_sub_.getTopic().c_str());
  }

  void SimpleImageSubNodelet::imgCallback(const sensor_msgs::ImageConstPtr &msg) {
